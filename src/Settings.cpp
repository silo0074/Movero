	  ...
	  ui(new Ui::Settings) // Initialise the member pointer `ui`
{
	...
	connect(ui->listWidgetMenu, &QListWidget::currentRowChanged, this, &Settings::onMenuPageChanged);
	ui->listWidgetMenu->setCurrentRow(0); // Default to General

	// Log Page Buttons
	connect(ui->btnRefreshLog, &QPushButton::clicked, this, &Settings::loadLogFile);
	connect(ui->btnOpenLogLocation, &QPushButton::clicked, this, &Settings::openLogLocation);
	connect(ui->btnClearLog, &QPushButton::clicked, this, &Settings::clearLogFile);
...
void Settings::onMenuPageChanged(int index) {
	ui->stackedWidget->setCurrentIndex(index);
	// Check if the new page is the Log page
	if (ui->listWidgetMenu->item(index)->text() == "Log") {
		loadLogFile();
	}
}

void Settings::loadLogFile() {
	QString logPath = LogManager::getLogFilePath();
	if (logPath.isEmpty()) {
		ui->plainTextLog->setPlainText(tr("Logging is not initialized."));
		return;
	}

	QFile logFile(logPath);
	if (!logFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
		ui->plainTextLog->setPlainText(tr("Could not open log file: %1").arg(logPath));
		return;
	}
	QTextStream in(&logFile);
	ui->plainTextLog->setPlainText(in.readAll());
	// Scroll to bottom
	ui->plainTextLog->verticalScrollBar()->setValue(ui->plainTextLog->verticalScrollBar()->maximum());
}

void Settings::openLogLocation() {
	QString logPath = LogManager::getLogFilePath();
